...
{
    private readonly ITipoDeduccionRepository _repo;

    public TipoDeduccionService(ITipoDeduccionRepository repo) => _repo = repo;

    public async Task<List<TipoDeduccionDto>> ListAsync(EstadoFilter filter = EstadoFilter.Activos, CancellationToken ct = default)
    {
        var items = await _repo.ListAsync(filter, ct);
        return items.Select(Map).ToList();
    }

    public async Task<TipoDeduccionDto?> GetByIdAsync(int id, CancellationToken ct = default)
    {
        var item = await _repo.GetByIdAsync(id, ct);
        return item is null ? null : Map(item);
    }

    public async Task<TipoDeduccionDto> CreateAsync(TipoDeduccionCreateDto dto, CancellationToken ct = default)
    {
        var porcentaje = NormalizePorcentaje(dto.DependeDeSalario, dto.Porcentaje);

        var entity = new TipoDeduccion
        {
    ...
    public async Task<TipoDeduccionDto?> UpdateAsync(int id, TipoDeduccionUpdateDto dto, CancellationToken ct = default)
    {
    ...
    private static TipoDeduccionDto Map(TipoDeduccion e) =>
